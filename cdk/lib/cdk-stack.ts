...
export class CdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const scheduleRule = new events.Rule(this, 'FetchHighestStockMoverSchedule', {
        schedule: events.Schedule.cron({ minute: '0', hour: '2' }) // trigger every 24 hours at 2 AM UTC / 6 PM PST (bumper time after stock market closes for Massive API to be available with the latest data)
    });
